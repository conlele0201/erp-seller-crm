    ...
    <div style={pageStyle}>
      ...
      <section style={statsGridStyle}>
        {stats.map((s) => (
          <div key={s.label} style={statCardStyle}>
            <div style={{ color: "#6b7280", marginBottom: 4 }}>{s.label}</div>
            <div style={{ fontSize: 24, fontWeight: 700 }}>{s.value}</div>
            <div style={{ fontSize: 12, color: "#16a34a" }}>{s.sub}</div>
          </div>
        ))}
      </section>
      ...
      >
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {loading ? (
              <tr><td style={tdStyle} colSpan={10}>Đang tải dữ liệu…</td></tr>
            ) : filteredProducts.length === 0 ? (
              <tr><td style={tdStyle} colSpan={10}>Không có sản phẩm nào.</td></tr>
            ) : (
